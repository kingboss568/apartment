...
struct GalleryView: View {
    var body: some View {
        NavigationStack { AllGalleryView() }
    }
}
...
private struct GalleryContentView: View {
    ...
    private var filters: [(id: String, title: String)] {
        var seen = Set<String>()
        let series = store.allChapters.compactMap { chapter -> (id: String, title: String)? in
            let id = chapter.seriesId
            guard seen.insert(id).inserted else { return nil }
            return (id, Constants.seriesName(forSeriesId: id))
        }
        return [("全部", "全部")] + series
    }
        ...
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 5) {
                Text("PINTEREST STYLE GALLERY")
                    .font(.system(size: 9, weight: .bold)).kerning(2).foregroundStyle(AppTheme.mute)
                Text(seriesId == nil ? "雙欄圖卡快覽" : title)
                    .font(.system(size: 27, weight: .heavy, design: .serif)).foregroundStyle(AppTheme.primary)
                Text("兩列並排・\(store.allArticles.count) 張本機圖解・可持續上下滑動")
                    .font(.system(size: 12, weight: .semibold)).foregroundStyle(AppTheme.mute)
            }
            Spacer()
            Image(systemName: "rectangle.grid.2x2.fill")
                .font(.system(size: 24, weight: .bold)).foregroundStyle(AppTheme.rust)
    ...
    private var searchField: some View {
        HStack(spacing: 10) {
            ...
            TextField("搜尋條號、標題或法規關鍵字", text: $query)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHStack(spacing: 8) {
                ForEach(filters, id: \.id) { filter in
                    Button {
                        withAnimation(.snappy(duration: 0.24)) { selectedSeries = filter.id }
                    } label: {
                        Text(filter.title)
                            ...
                            .padding(.horizontal, 12)
                            .frame(height: 34)
                            .background(selectedSeries == filter.id ? AppTheme.color(for: filter.id) : .white, in: Capsule())
    ...
    private var resultHeader: some View {
        HStack {
            Text(query.isEmpty ? "圖卡索引" : "搜尋結果")
                .font(.system(size: 17, weight: .heavy)).foregroundStyle(AppTheme.primary)
            Spacer()
            Text("\(visibleArticles.count) 張")
                .font(.system(size: 12, weight: .bold)).foregroundStyle(AppTheme.mute)
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            LazyVStack(spacing: 10) {
                ForEach(leftColumn) { PinterestArticleLink(article: $0) }
            }
            LazyVStack(spacing: 10) {
                ForEach(rightColumn) { PinterestArticleLink(article: $0) }
            }
        }
    }
}
...
private struct PinterestArticleLink: View {
    ...
    private var canAccess: Bool { iap.canAccess(article: article, allArticles: store.allArticles) }

    var body: some View {
        NavigationLink {
            if canAccess { ArticleDetailView(article: article) } else { PaywallView() }
        } label: {
                ...
                ZStack(alignment: .topTrailing) {
                    ...
                    if !canAccess {
                        ...
                        Image(systemName: "lock.fill")
                            ...
                            .padding(8).background(.black.opacity(0.35), in: Circle()).padding(7)
                    }
                }
                Text(article.article_no)
                    .font(.system(size: 10, weight: .black, design: .monospaced)).foregroundStyle(AppTheme.rust)
                Text(article.title_zh)
                    .font(.system(size: 14, weight: .heavy)).foregroundStyle(AppTheme.primary)
                    .multilineTextAlignment(.leading).lineLimit(3)
                Text(article.summary)
                    .font(.system(size: 11, weight: .medium)).foregroundStyle(AppTheme.mute)
                    .multilineTextAlignment(.leading).lineLimit(article.serial.isMultiple(of: 3) ? 3 : 2)
            }
            .padding(9)
