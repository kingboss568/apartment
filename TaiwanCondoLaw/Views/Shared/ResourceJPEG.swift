//
//  ResourceJPEG.swift
//  Loads a loose bundle image resource (HEIC first, JPG fallback).
//
//  We use this instead of `Image(name)` because SwiftUI's Image(name) only
//  finds Asset Catalog entries reliably; loose resources need explicit bundle URL lookup.
//
...
struct ResourceJPEG: View {
    ...
    let ext: String           // preferred extension
    ...
    private static func loadCached(name: String, ext: String) -> UIImage? {
        let extensions = preferredExtensions(ext)
        let key = "\(name).\(extensions.joined(separator: "|"))" as NSString
        if let cached = cache.object(forKey: key) {
            return cached
        }
        // Try multiple lookup strategies — works whether the image is in Asset Catalog,
        // at bundle root, or in an arbitrary subdirectory.
        if let img = UIImage(named: name) {
            cache.setObject(img, forKey: key)
            return img
        }
        for candidateExt in extensions {
        ...
        if let resourcePath = Bundle.main.resourcePath,
           let enumerator = FileManager.default.enumerator(atPath: resourcePath) {
            let targets = Set(extensions.map { "\(name).\($0)" })
    ...
    private static func preferredExtensions(_ ext: String) -> [String] {
        var seen = Set<String>()
        return ([ext, "heic", "jpg", "jpeg", "png"] as [String])
            .map { $0.lowercased() }
